...
use helix_core::{diagnostic::Severity, pos_at_coords, syntax, Range, Rope, Selection};
use helix_lsp::{
                    ...
                    Ok(MethodCall::ShowMessageRequest(params)) => {
                        if let Some(actions) = params.actions {
                            ...
                            let columns = [ui::PickerColumn::new(
                                "action",
                                |item: &MessageActionItem, _| item.title.as_str().into(),
                            )];
                            let picker = Picker::new(
                                ...
                                actions,
                                (),
                                move |cx, message_action, _action| {
                                    let lsp = match cx.editor.language_servers.get_by_id(server_id)
                                    {
                                        Some(language_server) => language_server.clone(),
                                        None => {
                                            warn!(
                                    ...
                                    tokio::spawn(async move {
                                        let res = lsp.reply(call_id, Ok(response));
                                        if let Err(res) = res {
                                            warn!("Failed to send command: {}", res);
                                        }
                                    });
                                },
                            );
